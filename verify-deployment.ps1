...
$health = Invoke-SSHCommand "curl -s http://localhost:3000/health 2>/dev/null" -Silent
if ($health) {
    try {
...
$authStatus = Invoke-SSHCommand "curl -s http://localhost:3000/auth/status 2>/dev/null" -Silent
if ($authStatus) {
    try {
        ...
        if ($authJson.authenticated) {
            ...
            Write-Host "  ⚠️  Not authenticated - visit: http://$VM_IP:3000/auth/login" -ForegroundColor Yellow
        }
    } catch {
...
$strategies = Invoke-SSHCommand "curl -s http://localhost:3000/strategies 2>/dev/null" -Silent
if ($strategies) {
    try {
...
$recentLogs = Invoke-SSHCommand "pm2 logs trading-bot-multi-strategy --lines 5 --nostream 2>/dev/null | tail -8" -Silent
if ($recentLogs) {
...
Write-Host "  Dashboard:      http://$VM_IP:3000/" -ForegroundColor White
Write-Host "  Health Check:   http://$VM_IP:3000/health" -ForegroundColor White
Write-Host "  Auth Login:     http://$VM_IP:3000/auth/login" -ForegroundColor White
Write-Host "  Strategies:     http://$VM_IP:3000/strategies" -ForegroundColor White
Write-Host ""
...
Write-Host "  View logs:      pm2 logs trading-bot-multi-strategy" -ForegroundColor DarkGray
Write-Host "  Restart:        pm2 restart trading-bot-multi-strategy" -ForegroundColor DarkGray
