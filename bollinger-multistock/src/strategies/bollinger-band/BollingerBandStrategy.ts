...
export class BollingerBandStrategy extends StrategyBase {
  ...
  private entryCutoffTimer?: NodeJS.Timeout;
  ...
  public async start(): Promise<void> {
    ...
    // Clear pending entries at 15:06 IST and close positions at 15:11 IST.
    this.scheduleEntryCutoff();
  ...
  private startMasterCycle(): void {
    ...
    const fetchCandleIfMarketOpen = async () => {
      ...
      if (isUnderlyingContinuousSession(now)) {
        // Detect sleep disruption only while candles are expected. Checking after
        // 15:15 would incorrectly report the intentional CAS pause as an outage.
  ...
  private isMarketHours(): boolean {
    return isUnderlyingContinuousSession();
  }
  ...
  private async checkEntrySignals(): Promise<void> {
    ...
    if (!this.isMarketHours()) {
      this.logger.debug('🔒 Signal check skipped - Outside underlying continuous session (9:15 AM - 3:15 PM IST)');
      return;
    }
    ...
    if (!isEntryAllowed()) {
      this.clearPendingEntryStates('15:06 IST entry cutoff');
      this.logger.debug('🔒 Entry blocked - 15:06 IST entry cutoff reached');
      return;
    }
    ...
    // so a late-day setup can still convert before the 15:06 IST entry cutoff. The original cutoff
    ...
    const isFirstCandleWindow = currentMinutes >= (9 * 60 + 15) && currentMinutes < (9 * 60 + 25);
  ...
  private clearPendingEntryStates(reason: string): void {
    let changed = false;

    if (this.currentArmedSignal) {
      this.logSignalLifecycle('ABANDONED', this.currentArmedSignal.direction, reason);
      this.currentArmedSignal = null;
      changed = true;
    }

    if (this.currentFvgWatch) {
      this.logSignalLifecycle('FVG_INVALIDATED', 'LONG', reason);
      this.currentFvgWatch = null;
      changed = true;
    }
    ...
    if (changed) {
      this.logger.info(`🔒 Pending entry states cleared: ${reason}`, {
        symbol: this.signalSymbol,
        slot: this.slotIndex + 1,
      });
      this.saveCapitalData();
  ...
  private scheduleEntryCutoff(): void {
    this.cancelEntryCutoff();
    const delay = millisecondsUntilIstTimeToday(MARKET_TIMES_IST.entryCutoff);
    ...
    this.entryCutoffTimer = setTimeout(() => {
      this.logger.info('🔒 15:06 IST entry cutoff reached');
      this.clearPendingEntryStates('15:06 IST entry cutoff');
    }, delay);
  }

  private cancelEntryCutoff(): void {
    if (this.entryCutoffTimer) {
      clearTimeout(this.entryCutoffTimer);
      delete this.entryCutoffTimer;
    }
  }

  /**
   * Close before the underlying enters CAS and before the 15:25 MIS square-off.
   */
  private scheduleEODExit(): void {
    ...
    const delay = millisecondsUntilIstTimeToday(MARKET_TIMES_IST.eodExit);
    if (delay !== null) {
      this.logger.info(`📅 EOD safety exit scheduled for 3:11 PM IST (in ${Math.round(delay / 60000)} minutes)`);

      this.eodExitTimer = setTimeout(async () => {
        if (this.currentPosition) {
          this.logger.warn('🕒 EOD safety exit triggered at 3:11 PM IST');
          await this.forceClosePosition('EOD_SAFETY_EXIT_3:11PM');
        } else {
          this.logger.info('✅ No active position at 3:11 PM IST, no EOD exit needed');
        }
      }, delay);
    } else {
      const now = new Date();

      if (this.currentPosition && isEodRecoveryWindow(now)) {
        this.logger.warn('🕒 Recovered active position after 3:11 PM IST; forcing immediate EOD exit');
        this.eodExitTimer = setTimeout(async () => {
          if (this.currentPosition) {
            await this.forceClosePosition('EOD_SAFETY_EXIT_3:11PM');
          }
        ...
        this.logger.info('⏰ Already past 3:11 PM IST today, no EOD exit scheduled');
      }
    }
  }
  ...
  private async executeOrder(transaction: 'BUY' | 'SELL', instrument: any, quantity: number): Promise<{ success: boolean; price: number; orderId?: string }> {
    try {
      ...
      if (transaction === 'BUY' && !isEntryAllowed()) {
        this.logger.warn('🔒 ENTRY_REJECTED: 15:06 IST cutoff reached before order placement', {
          instrument: instrument.tradingsymbol,
        });
        this.logSignalLifecycle('ENTRY_REJECTED', inferredDirection, '15:06 IST entry cutoff');
        return { success: false, price: 0 };
      }
