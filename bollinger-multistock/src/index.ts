...
import { BrokerDataGateway } from './services/BrokerDataGateway';
import { SwingServiceProxy } from './services/SwingServiceProxy';
import { renderSwingDashboard } from './ui/SwingDashboard';
import { renderSectorRotationDashboard } from './ui/SectorRotationDashboard';
import { renderWatchlistDashboard } from './ui/WatchlistDashboard';
...
class TradingBot {
  ...
  private brokerDataGateway: BrokerDataGateway;
  private swingServiceProxy: SwingServiceProxy;
  ...
  constructor() {
    ...
    this.brokerDataGateway = new BrokerDataGateway(this.kiteConnect, this.authService, this.logger);
    this.swingServiceProxy = new SwingServiceProxy();
  ...
  private setupRoutes(): void {
    ...
    // =============================
    // SWING SCANNER — COMMON AUTH GATEWAY
    // =============================

    this.app.get('/swing', async (_req: Request, res: Response): Promise<void> => {
      const authenticated = this.authService.isAuthenticated()
        && await this.authService.isAuthenticatedAndValid();
      res.status(200).send(renderSwingDashboard(
        this.basePath,
        authenticated,
        this.authService.getSessionData()?.user_name,
      ));
    });

    this.app.get('/swing/sectors', async (_req: Request, res: Response): Promise<void> => {
      const authenticated = this.authService.isAuthenticated()
        && await this.authService.isAuthenticatedAndValid();
      res.status(200).send(renderSectorRotationDashboard(
        this.basePath,
        authenticated,
        this.authService.getSessionData()?.user_name,
      ));
    ...
    this.app.get('/swing/watchlist/secondary', async (_req: Request, res: Response): Promise<void> => {
      const authenticated = this.authService.isAuthenticated() && await this.authService.isAuthenticatedAndValid();
      res.status(200).send(renderWatchlistDashboard(this.basePath, authenticated, 'SECONDARY', this.authService.getSessionData()?.user_name));
    });

    this.app.get('/swing/watchlist/primary', async (_req: Request, res: Response): Promise<void> => {
      const authenticated = this.authService.isAuthenticated() && await this.authService.isAuthenticatedAndValid();
      res.status(200).send(renderWatchlistDashboard(this.basePath, authenticated, 'PRIMARY', this.authService.getSessionData()?.user_name));
    });

    this.app.get('/api/swing/status', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/status');
    });

    this.app.get('/api/swing/results/latest', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/results/latest');
    });

    this.app.get('/api/swing/gates', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/gates');
    });

    this.app.get('/api/swing/config', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/config');
    });

    this.app.get('/api/swing/sectors', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/sectors');
    });

    this.app.get('/api/swing/sectors/rotation', async (req: Request, res: Response): Promise<void> => {
      const weeks = typeof req.query.weeks === 'string' ? encodeURIComponent(req.query.weeks) : '8';
      await this.proxySwingRequest(res, `/api/sectors/rotation?weeks=${weeks}`);
    });

    this.app.post('/api/swing/scan', async (req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/scan', 'POST', req.body);
    });

    this.app.get('/api/swing/corporate-actions/qc', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/corporate-actions/qc');
    });

    this.app.post('/api/swing/corporate-actions/qc', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/corporate-actions/qc', 'POST', {});
    });

    this.app.get('/api/swing/watchlists/status', async (_req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/watchlists/status');
    });

    this.app.get('/api/swing/watchlists/:state', async (req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, `/api/watchlists/${encodeURIComponent(req.params.state ?? '')}`);
    });

    this.app.post('/api/swing/watchlists/secondary/add', async (req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, '/api/watchlists/secondary/add', 'POST', req.body);
    });

    this.app.post('/api/swing/watchlists/:symbol/state', async (req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, `/api/watchlists/${encodeURIComponent(req.params.symbol ?? '')}/state`, 'POST', req.body);
    });

    this.app.post('/api/swing/watchlists/:symbol/details', async (req: Request, res: Response): Promise<void> => {
      await this.proxySwingRequest(res, `/api/watchlists/${encodeURIComponent(req.params.symbol ?? '')}/details`, 'POST', req.body);
    });
...
<body>
    <div class="container">
        <div class="header">
            <div class="header-top">
                ...
                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; justify-content: flex-end;">
                  <a href="${this.basePath}/" class="btn btn-primary">Bollinger Intraday</a>
                  <a href="${this.basePath}/swing" class="btn btn-secondary">↗ Swing Scanner</a>
          ...
          <div class="scanner-section" id="swingSummary" style="border-left: 4px solid #7c3aed;">
            <div class="section-header">
              <div class="section-title"><span>↗</span> Swing Trading Scanner</div>
              <a href="${this.basePath}/swing" class="btn btn-secondary">Open Swing Workspace</a>
            </div>
            <div id="swingSummaryContent" style="padding: 18px; color: #64748b;">
              Loading independent daily scanner status…
            </div>
          </div>
        ...
        <div class="scanner-section">
            ...
            <div class="scanner-timeline">
                ...
                <div class="timeline-item upcoming">
                  <div class="timeline-time">15:06 / 15:11</div>
                  <div class="timeline-label">Entry cutoff / EOD position exit</div>
                </div>
                <div class="timeline-item upcoming">
                  <div class="timeline-time">15:41</div>
                  <div class="timeline-label">Post-derivatives cleanup</div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function loadSwingSummary() {
          const container = document.getElementById('swingSummaryContent');
          if (!container) return;
          try {
            const response = await fetch('${this.basePath}/api/swing/results/latest');
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Swing service unavailable');
            const blocked = Array.isArray(data.blocked) ? data.blocked : [];
            container.innerHTML = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px">'
              + '<div><strong style="display:block;color:#172033;font-size:1.25rem">' + (data.asOfDate || 'Never') + '</strong><span>Latest scan</span></div>'
              + '<div><strong style="display:block;color:#172033;font-size:1.25rem">' + (data.universeCount || 0) + '</strong><span>Universe</span></div>'
              + '<div><strong style="display:block;color:#172033;font-size:1.25rem">' + (data.passedCount || 0) + '</strong><span>Qualified</span></div>'
              + '<div><strong style="display:block;color:' + (blocked.length ? '#b91c1c' : '#15803d') + ';font-size:1.25rem">' + blocked.length + '</strong><span>Data blockers</span></div>'
              + '</div>';
          } catch (error) {
            container.style.color = '#b91c1c';
            container.textContent = 'Swing scanner offline: ' + String(error.message || error);
          }
        }
        loadSwingSummary();
...
<body>
      ...
      <div style="margin-top: 16px; padding: 14px; background: #f8fafc; border-radius: 10px; border: 1px solid #e2e8f0;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 0.85rem;">
          ...
          <div class="position-detail-row">
            ...
            <span class="position-detail-value" style="color: #10b981;">✅ 3:11 PM IST</span>
          </div>
        </div>
      </div>
    </div>
  ...
  public async start(): Promise<void> {
    try {
      ...
      await this.brokerDataGateway.start();
  ...
  public async gracefulShutdown(): Promise<void> {
    ...
    try {
      await this.brokerDataGateway.stop();
  ...
  private async proxySwingRequest(res: Response, path: string, method: 'GET' | 'POST' = 'GET', body?: unknown): Promise<void> {
    const authenticated = this.authService.isAuthenticated()
      && await this.authService.isAuthenticatedAndValid();
    if (!authenticated) {
      res.status(401).json({ error: 'Authenticate once through the common Zerodha login before using Swing' });
      return;
    }
    const response = await this.swingServiceProxy.request(path, method, body);
