...
export class StrategyManager {
  ...
  private schedulePreMarketCheck(): void {
    ...
    this.preMarketCheckInterval = setInterval(async () => {
      const now = new Date();
      const currentTime = getIstMinutes(now);

      // Clean up only after the 15:40 IST derivatives close.
      if (currentTime === MARKET_TIMES_IST.cleanup) {
  ...
  /**
   * Gate underlying candle polling to the continuous cash session. CAS observations
   * after 15:15 IST must not be appended as ordinary five-minute candles.
   */
