...
- **15:06 entry cutoff**: blocks all new BUY orders and clears persisted pullback/FVG entry states.
- **15:15 underlying cutoff**: stops continuous-session candle polling before CAS data begins.
- **Post-market cleanup 15:41**: clears cache, slot states, and cooldowns after derivatives close.
...
| --- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | --------------------------------------- |
| 1   | **EOD safety**             | **15:11 IST** one-shot timer (before the 15:15 underlying CAS transition and 15:25 broker square-off)                                                                    | option | **Always on**                           |
...
**In practice**: the two dominant exit families are **`LONG_SUPERTREND_BREAK`** and the EOD safety exit. Historical records use `EOD_SAFETY_EXIT_3:19PM`; new records use **`EOD_SAFETY_EXIT_3:11PM`**. Layers 7–9 effectively never fire because they're off.
