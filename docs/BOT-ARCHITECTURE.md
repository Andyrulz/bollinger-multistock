...
| **Exit**            | EOD exit time                        | 3:19 PM                  | Auto-exit before broker squareoff     |
...
└── EOD timer: Fires at 3:19 PM (unconditional exit)
```

### Phase 7: End of Day (3:19 PM – 3:40 PM)

```
3:19 PM  → EOD safety exit: Force-close all positions
3:25 PM  → Cache historical data for tomorrow's startup
3:40 PM  → OIHistoryService.saveEndOfDayOI()
           → Save current OI for tomorrow's Smart Money scoring
```
