...
| --- | ---------------------------- | ------------------------------------ | ---------------------------------------------------- | --------------------------- | ---------- | -------- |
| 1   | EOD Safety Exit              | `EOD_SAFETY_EXIT_3:19PM`             | Clock hits 3:19 PM                                   | Once/day (scheduled timer)  | ALL        | Highest  |
...
**Purpose**: Close all positions before the broker's MIS auto-squareoff window (~3:25 PM) to avoid squareoff charges (₹50+ per order).

**How it works**:

- A `setTimeout` is scheduled at strategy start targeting **3:19 PM** (6-minute buffer before broker cutoff).
- When the timer fires, if a position exists, `forceClosePosition('EOD_SAFETY_EXIT_3:19PM')` is called.
- If no position is open at 3:19 PM, it logs and does nothing.

**Configuration**:
| Parameter | Value |
|-----------|-------|
| Exit time | 3:19 PM IST |
| Buffer before broker squareoff | 6 minutes |
...
│ EOD Safety Timer                │ → fires once at 3:19 PM
